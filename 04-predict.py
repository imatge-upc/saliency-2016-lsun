...
if __name__ == "__main__":
    ...
    for currChunk in chunks(validationData, batchSize):
        ...
        idx=idx+batchSize
    '''
    with open( 'results_and_names.pickle', 'wb' ) as f:
        data_to_save = ( y_pred, listImages )
        pickle.dump( data_to_save, f )   

    f = file( 'results_and_names.pickle' , 'rb' )
    results , names = pickle.load( f )
    f.close()
    '''
    for res, name in zip(y_pred,listImages):
        ...
        scipy.io.savemat('/imatge/jpan/work/lsun2016/results/10-04-2016/'+name,saliencyMapMatlab )