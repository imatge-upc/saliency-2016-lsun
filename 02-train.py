    ...
    for currEpoch in tqdm(range(numEpochs), ncols=20):
        ...
        for currChunk in chunks(trainData, batchSize):
            ...
            for k in range( batchSize ):
                batchIn[k,...] = (currChunk[k].image.data.astype(theano.config.floatX).transpose(2,0,1)-model.meanImage_VGG_ImageNet)/255.
        ...
        np.savez( "./model/modelWeights{:04d}.npz".format(currEpoch), *lasagne.layers.get_all_param_values(model.net['output']))