...
if __name__ == "__main__":
	...
	with open( 'trainDataLite.pickle', 'rb') as f:
	...
	lasagne.layers.set_all_param_values(inputNetwork['output'], param_values)
	...
	for currChunk in tqdm(chunks(validationData, batchSize), ncols=30, total=len(validationData)/batchSize):
	
		batchInPlaces = np.zeros((len(currChunk), 3, inputHeight, inputWidth ), theano.config.floatX )
		batchInFaces = np.zeros((len(currChunk), 3, inputHeight, inputWidth ), theano.config.floatX )
		batchInImageNet = np.zeros((len(currChunk), 3, inputHeight, inputWidth ), theano.config.floatX )
		...
		result = predictFunction( batchInPlaces, batchInFaces, batchInImageNet )
		print result.shape
		break
